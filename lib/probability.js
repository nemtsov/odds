...
Probability.prototype._ofSet = function (set) {
  ...
  var spaceAndSet = this.space.intersect(set);
  return spaceAndSet.length / this.space.length;
};
...
Probability.prototype._ofSetGiven = function (set, condition) {
  var conditionAndSet = condition.intersect(set);
  return this._ofSet(conditionAndSet) / this._ofSet(condition);
};
